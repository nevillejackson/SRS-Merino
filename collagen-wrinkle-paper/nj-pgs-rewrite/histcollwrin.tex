...
The total number of pixels in an image taken with a 40x objective was 1920000, so one could scale these optical density sums to the average optical density of a pixel by dividing by 1920000.
...
The residual term in Figure~\ref{tab:redpixelt1aov} is the variation between randomly chosen Fields within a specimen, because there are no replicate sheep within each Flock:SkinType subclass. The difference between the wrinkled and wrinkle-free SkinTypes is shown to be highly significant. The Flock differences are also significant, and there is an interaction.
...
The difference between wrinkled and wrinkle-free SkinTypes is again shown to be highly significant. The Flock differences are also significant and there is a large interaction of Flock with SkinType. The two Flocks in Trial 2 were quite diffferent. Flock 6 had a much wider range of SkinTypes from which to choose sheep. This is reflected in a much wider difference in amount of collagen between the wrinkled and wrinkle-free sheep in Flock 6. 

 The on-wrinkle and between-wrinkle sampling positions within the wrinkled specimens were significantly different but the size of the difference was only 33257 units and was in the opposite of the expected direction. The on-wrinkle specimens actually had a lower integrated optical density than the between-wrinkle specimens indicating slightly {\em less} collagen on a wrinkle than between wrinkles.

\begin{verbatim}
There are some questions hanging over these data.
The data can not be pixel counts, because they are not integers. 
They must be something like counts weighted by the intensity of redness
 of each pixel, or something similar.
