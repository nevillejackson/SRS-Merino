...
\item sheep with more or harder collagen will have lower pp-cell numbers,with consequent lower S/P ratio and higher S fibre diameters.
\end{itemize}
...
\input{tabcstaov.tex}  
In this case both the Flock and SkinType effects are significant, but not the interaction. So we conclude that thickness of collagen in the dermis after compression is greater in wrinkled sheep ({\em tight} grade), and is reduced in sheep with loose skin ({\em SRS} grade).
...
\input{tabcmpaov.tex}
As for CST the Flock and SkinType effects are significant, but not the interaction. So we conclude that compressibility is least in wrinkled sheep ({\em tight} grade), and is greatest in sheep with loose skin ({\em SRS} grade). So the collagen is harder in wrinkled sheep, and softer in sheep with loose skin.
...
The correlation is 0.39 and is significant $(P<0.01)$. In this case there little suggestion of Flock 4 being higher in DsSD than expected from CSTmean. So in this case other factors are not operating. It is likely that the effect of collagen on DsSD is not via mean Ds, but is a conseuence of the {\em wrinkle - hard collagen - curved follicles - curved fibres link}. It is known that skins with high follicle curvature score also have high follicle unevenness score (genetic correlation is $0.82\pm0.02$ Jackson(2017)~\cite{jack:17a}), and therefore grow fibres of less uniform diameter.
...
The correlation is -0.43 and is significant $(P<0.01)$.  Again Flock 4 does not stand out, so the remarks above apply. Hard collagen makes curved follicles, which are uneven in their anatomy and in their fibre growth. 

What should be looked at here is the relations of collagen measurements to follicle curvature and follicle unevenness scores. We do not have the data. 
...
[ An alternative would be to get follicle curvature and unevenness ( and probably depth) on the current data, where we have the actual collagen measurements. Is that possible?]
